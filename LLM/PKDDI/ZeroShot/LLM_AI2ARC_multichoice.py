from datapreprocessing import myDataSet, AI2ARC_Challenge_qaoptions_Processor as Processor
...
parser.add_argument('--run', dest='RunIndex', default = 3, type=str, help='SGE_TASK_ID')
...
def collate_tokenize(data): #data is a list of length batch_size (defined in DataLoader later;
  ...
  text_batch = [element[0] for element in data] #if preceding with leading examples
...
eval_examples = processor.get_dev_examples('../../Dataset/TACDDI_nomask_dev_nFood')
...
#            **encodings, #encodings are (list of) BatchEncoding objects; and ** seems to be the way to give it to generate
#            num_beams=2, #the "force_words_ids" thing seems to be beam search
##            num_return_sequences=2, #in theory better return multiple choices because some (most) of them don't even have the force words; but found that returning 1 often is correct?
##            early_stopping=True,
#            #output_scores=False,
#            #force_words_ids=force_words_ids,
##            do_sample=True, #turn off sampling the next word; use default greedy algorithm to pick the highest possible next word
#            top_p=0.9,
#            temperature=0.9,
#            max_new_tokens=15,
...
#        new_output = beam_output[:,(size_of_full_tokens - 17):]
        ...
        for i, label in enumerate(truelabels): #truelabels is a tensor of true labels for this batch
            ...
            if Think_location >= 0:
                ...
                if len(matches)==1:
                    if predicted[matches[1]:].lower().find("no")>=0:
                        predicted_label='No'
                    elif predicted[matches[1]:].lower().find("yes")>=0:
                        predicted_label='Yes'
                else:
                    if label=='Yes':
                            predicted_label='No'
...
OUTPUT_file = 'FewShot_NoSampling' + '.' + str(RunIndex) + '.txt'
...
for idx in range(1,1001):
    ...
    resampled_indices, tmp = Sampling.fit_resample(torch.tensor(indices).reshape(-1, 1), y_true)
    Y_TRUE = []
    Y_PRED = []
    ...
    output_file2 = 'AI2ARC_validation_under_results_' + str(idx) + ".txt"
    output_predict_file = os.path.join("AI2ARC_validation_results/", output_file2)
    with open(output_predict_file,'w') as writer:
        for sampleIdx, prediction in enumerate(Y_PRED):
            output_line = str(Y_TRUE[sampleIdx])+"\t"+ str(prediction)+ "\n"
            writer.write(output_line)
